...
while i<=length(x_t)
    ...
    hold on
    check = search(x_a(i),y_a(i), target_pos_x(i), target_pos_y(i),visibility{ud},visibility1{ud},visibility2{ud},cent(ud,1),cent(ud,2),a(ud,:),b(ud,:),c(ud,:),d(ud,:),ud,h,sensor_range);
    if(check == 1)
...
while i<=length(x_t)
    ...
    plot3(x_a(i),y_a(i),h_uav(i),'r');
    hold on
    check = search(x_a(i),y_a(i), target_pos_x(i), target_pos_y(i),visibility{ud},visibility1{ud},visibility2{ud},a(ud,:),b(ud,:),c(ud,:),d(ud,:),ud,h,sensor_range);
    if(check == 1)
        disp('Eureka!! Target Found');
        plot3(x_a(i), y_a(i),h,'o','MarkerSize',8,'MarkerFaceColor','magenta');
        gta = 7;
